defmodule Mix.Tasks.Rheo.Bench do
  ...
  defp consume_all(_stream, remaining) when remaining <= 0, do: :ok

  defp consume_all(stream, remaining) do
    limit = min(remaining, 50)
    {:ok, leases} = Rheo.fetch(stream, "g", limit: limit, rheo: Rheo.Bench)
    Enum.each(leases, &Rheo.ack(&1, rheo: Rheo.Bench))
    consume_all(stream, remaining - length(leases))
  end
