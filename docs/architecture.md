...
Rheo **v0.5.0** is an embedded Elixir/OTP library. Durable truth lives in the
...
| Per-partition sequence | `streams.next_sequences` | Monotonic per partition |
...
events (ADR 015). Partitions use per-partition sequences and a contiguous ACK
frontier; see `Rheo.lag/3` and ADR 016.
...
- Migrations: [0.4 → 0.5](https://hexdocs.pm/rheo/0-4-to-0-5.html) ·
